...
class LibvirtStorPoolVolumeDriver(LibvirtBaseVolumeDriver):
    def __init__(self, connection):
        try:
            from storpool import spopenstack
        except ImportError:
            LOG.error(_LE('Could not import the StorPool API bindings'))
            return

        super(LibvirtStorPoolVolumeDriver, self).__init__(connection,
                                                       is_block_dev=True)
        self._spopenstack = spopenstack
        self._attach = self._spopenstack.AttachDB(log=LOG)
        self._attach.api()

    def get_config(self, connection_info, disk_info):
        """Returns xml for libvirt."""
        volume_id = connection_info['serial']
        name = self._attach.volumeName(volume_id)
        conf = super(LibvirtStorPoolVolumeDriver, self).get_config(
            connection_info, disk_info)
        conf.source_type = 'block'
        conf.source_path = '/dev/storpool/' + name
        return conf

    def connect_volume(self, connection_info, disk_info):
        volume_id = connection_info['serial']
        req_id = 'nova-attach--{id}'.format(id=volume_id)
        name = self._attach.volumeName(volume_id)
