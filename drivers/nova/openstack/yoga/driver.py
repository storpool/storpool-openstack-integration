...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _connect_volume(self, context, connection_info, instance,
        ...
        for bdm in objects.BlockDeviceMappingList.get_by_instance_uuid(context, instance.uuid):
            if 'volume_id' in bdm and bdm['volume_id'] == connection_info['data']['volume']:
                if 'device_name' in bdm:
                    LOG.debug("Volume: %(vol)s is attached at %(dev)s on VM %(vm)s", {'vol': bdm['volume_id'], 'dev': bdm['device_name'], 'vm': instance.uuid})
                    device_name = bdm['device_name']

        if device_name is None:
            LOG.debug("Could not find the device name for volume %(vol) at VM %(vm)s", {'vol': connection_info['data']['volume'], 'vm': instance.uuid})
